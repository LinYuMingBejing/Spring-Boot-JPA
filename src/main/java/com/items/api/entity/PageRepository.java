...
public interface PageRepository extends JpaRepository<Page, Integer> {
    ...
    @Query(value = "SELECT * FROM page_location_device_system_pv_20200608 order by pv_valid desc", nativeQuery = true)
    List<Page> findByPV(Pageable pageable); // 抓出PV前100名文章

    @Query(value = "SELECT * FROM page_location_device_system_pv_20200608 where pv_valid >5", nativeQuery = true)
    List<Page> findByHighPv(Pageable pageable); // 抓出PV前100名文章

    @Query(value = "SELECT * FROM page_location_device_system_pv_20200608 where city = ?", nativeQuery = true)
